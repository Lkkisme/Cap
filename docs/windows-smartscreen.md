...
1. Microsoft Store
   ...
   运行 GitHub Actions 中的 `Windows Store Package` workflow 可以生成 Store 用 EXE/MSI 离线安装包。该 workflow 始终要求 Windows Authenticode 签名已配置，并会校验 `store-submission-package` 里的每个安装包签名有效，不再产出 unsigned Store installer artifact。workflow 还会生成 `store-submission-package`，把 Partner Center 需要的包 URL、架构、语言、静默安装参数、SHA256、签名状态和发布者信息整理成 checklist、JSON 和 CSV。
