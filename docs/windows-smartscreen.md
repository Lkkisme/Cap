...
- Windows Release、Windows Store EXE/MSI 和 Windows MSIX Store 构建都会设置 `VITE_DISABLE_UPDATER=true`，桌面端不会尝试旧的内置 updater；普通生产 Tauri 配置也不再指向上游 CrabNebula updater。更新页和设置页会把用户带到本仓库 Releases，web dashboard 的 Download App 菜单会打开当前站点 `/download`，避免绕过 Store 优先和 Release 证据门禁。
