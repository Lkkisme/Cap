...
当前只推进 Windows 和 SignPath Foundation：

1. 公开补齐 README、隐私政策、Code signing policy、SignPath 申请草稿和资格自查表。
2. SignPath Foundation 免费开源代码签名申请已在 2026-06-21 提交，当前等待审核。
3. 申请通过后，把 GitHub Actions 配成 SignPath trusted build system。
4. 用 SignPath Foundation 签名新的 Windows EXE、MSI 和 portable ZIP 内部 EXE/DLL。
5. 验证 Authenticode 状态为 `Valid`，发布者为 SignPath Foundation，并保留 Release audit、安装器 smoke test、Defender、WDSI 和 checksum 证据。

Microsoft Store、Azure Artifact Signing 和 PFX 仍是技术备选，但不是当前目标。

## 最有效路线

1. SignPath Foundation

   SignPath 是适合 CI 的签名服务，开源项目可以申请 SignPath Foundation。仓库已有 `.github/signpath/artifact-configuration.xml`，会对 Windows 主程序、符合项目元数据限制的 DLL 和 GitHub Release 中的 Windows EXE/MSI 执行 Authenticode 签名；portable ZIP 会在主程序/DLL 已签名后生成。
   ...
   - [signpath-foundation-status.md](signpath-foundation-status.md)
...
2. Microsoft Store
   ...
   运行 GitHub Actions 中的 `Windows Store Package` workflow 可以生成 Store 用 EXE/MSI 离线安装包。该 workflow 始终要求 Windows Authenticode 签名已配置，并会校验 `store-submission-package` 里的每个安装包签名有效，不再产出 unsigned Store installer artifact。workflow 还会生成 `store-submission-package`，把 Partner Center 需要的包 URL、架构、语言、静默安装参数、SHA256、签名状态和发布者信息整理成 checklist、JSON、CSV 和 `microsoft-store-product-update.json`。如果 EXE/MSI 产品已经在 Partner Center 创建并完成过首次提交，并且签名安装包已上传到你自己的版本化 HTTPS CDN 或网站路径，可以把 workflow 的 `publish_to_store` 设为 `true`，填写 `store_product_id` 和 `package_url_base`，让 `microsoft/store-submission` 自动更新并提交 Store 草稿包。
...
3. Azure Artifact Signing（formerly Trusted Signing）
   ...
   微软推荐非 Store 分发使用 Azure Artifact Signing（formerly Trusted Signing）。它不需要本地硬件 token，能在 GitHub Actions 中签名，当前基础套餐约为每月 9.99 美元。签名后 Windows 会显示已验证发布者，并且后续版本可以积累发布者声誉。发布安装包必须带可信时间戳，否则证书过期或吊销后的长期验证会变差，本仓库的正式发布、审计、WinGet 和 WDSI 流程都会拒绝缺少时间戳的安装包。
...
`Windows Release` workflow 始终要求 Windows Authenticode 签名。通过 `cap-v*` tag 触发或手动运行都不能关闭签名要求；手动运行 workflow 时只能选择是否保持 draft，不能生成未签名 Windows 安装包。启用签名时必须配置 `WINDOWS_SIGNING_PUBLISHER_PATTERN`，让 workflow 在签名后确认 Authenticode subject 是预期发布者。把 `WINDOWS_SIGNING_PROVIDER` 设置为 `azure-artifact-signing` 或 `pfx` 后，Windows 构建会：

1. 先构建未打包的 Windows 主程序
2. 签名主程序和 DLL
3. 再生成 NSIS EXE 和 MSI 安装包
4. 再签名最终上传到 GitHub Release 的 EXE/MSI，并用已签名主程序/DLL 生成 portable ZIP

把 `WINDOWS_SIGNING_PROVIDER` 设置为 `signpath` 后，workflow 会先构建未打包的 Windows 主程序，把主程序和 DLL 提交到 SignPath 签名并验证，再生成 NSIS EXE 和 MSI 安装包，然后把 EXE/MSI 再提交到 SignPath 签名，最后验证签名状态、可信时间戳和 `signtool verify /pa /tw` 结果。portable ZIP 会在这些签名验证通过后生成，并在审计时展开检查内部 EXE/DLL。

在 GitHub 仓库中添加这些 Variables：

| Name | Example |
| --- | --- |
| `WINDOWS_DOWNLOAD_URL` | Official Microsoft Store URL, or your deployed `/download/windows` URL |
| `WINDOWS_PREVIOUS_VERSIONS_URL` | Optional deployed `/download/versions` URL |
| `WINDOWS_PUBLIC_WEB_URL` | Optional public web/API base URL for the Windows desktop build |
| `WINDOWS_RELEASE_ASSET_BASE_URL` | Optional project-controlled HTTPS mirror for signed Release assets, e.g. `https://downloads.example.com/cap/{tag}/{filename}` |
| `WINDOWS_SIGNING_PROVIDER` | `azure-artifact-signing`, `trusted-signing`, `azure-trusted-signing`, `signpath`, or `pfx` |
...
Azure Artifact Signing（formerly Trusted Signing）需要这些 Secrets：
...
- 优先推广 Microsoft Store 链接；Store 暂未可用时，再推广同一批已签名安装包的自有 HTTPS 下载域名，让同一文件 hash 和同一个项目控制的下载入口积累声誉。Web 下载入口中的 `/download/windows` 会在配置 `NEXT_PUBLIC_WINDOWS_STORE_URL`、`WINDOWS_STORE_URL` 或 `CAP_WINDOWS_STORE_URL` 后优先跳到项目配置的 Microsoft Store HTTPS 详情页或应用页链接；未配置 Store 链接时，`/download/windows`、`/download/windows-exe`、`/download/windows-msi` 和 `/download/windows-zip` 只会直跳同时带 Windows EXE、MSI、portable ZIP、`SHA256SUMS.txt`、当前 tag 对应的 `windows-smartscreen-report-<tag>.md`、`windows-release-assets-<tag>.json`、`windows-installer-smoke-test-report-<tag>.md`、`windows-installer-smoke-test-results-<tag>.json`、`windows-winget-manifest-<tag>.zip`、`windows-winget-submission-<tag>.md`、`windows-wdsi-submission-checklist-<tag>.md` 和 `windows-wdsi-submission-text-<tag>.zip`，并且 `windows-release-assets-<tag>.json` 中每个 Windows 安装包和 portable ZIP 的签名、时间戳、SignTool、checksum、attestation 和 Defender 状态都有效的 Windows 资产；配置 `WINDOWS_RELEASE_ASSET_BASE_URL`、`CAP_WINDOWS_RELEASE_ASSET_BASE_URL` 或 `NEXT_PUBLIC_WINDOWS_RELEASE_ASSET_BASE_URL` 后，这些已验证 Windows 资产会优先使用你自己的版本化 HTTPS CDN 或网站路径。该变量支持 `{tag}` 和 `{filename}` 模板，并会拒绝 GitHub Releases、localhost 和上游 `cap.so`。如果当前最新公开 Release 仍是未签名旧包、缺少审计证据、缺少安装器 smoke test 证据、缺少 WinGet manifest、缺少 WDSI 复核材料或证据清单内容无效，则进入 `/download/windows-status` 而不是自动下载或跳到 GitHub Releases。All Versions 页面也只会给证据齐全且清单内容有效的 Windows Release 展示 EXE/MSI/ZIP 直链。
...
- Windows Release、Windows Store EXE/MSI 和 Windows MSIX Store 构建都会设置 `VITE_DISABLE_UPDATER=true`，桌面端不会尝试旧的内置 updater；普通生产 Tauri 配置也不再指向上游 CrabNebula updater。更新页、设置页和 web dashboard 的 Download App 菜单会打开当前站点 `/download` 或 `/download/versions`，避免绕过 Store 优先和 Release 证据门禁。
- 已公开的 `cap-v*` Release 不要替换 EXE/MSI/portable ZIP；如果需要重新发布 Windows 包，创建新的 tag，让用户和微软都能看到清晰版本边界。
- 保留源码、release notes、hash、签名信息，方便微软人工复核。
- 发布后下载 Windows EXE/MSI/portable ZIP；EXE/MSI 可直接用 `Get-AuthenticodeSignature` 确认状态是 `Valid`，ZIP 解压后对里面的 `Cap-CN.exe` 和 DLL 做同样检查，确认存在 `TimeStamperCertificate`，并用 Windows SDK `signtool verify /pa /tw` 复核。
- 发布后等待自动触发的 `Windows Release Audit` workflow 通过，确认 Release 中的 Windows EXE/MSI/portable ZIP 签名发布者匹配、带可信时间戳、通过 SignTool 复核、匹配 `SHA256SUMS.txt`、通过 GitHub artifact attestation 验证，并且重新下载后通过 Microsoft Defender 扫描。portable ZIP 的签名状态来自内部 EXE/DLL。该 workflow 通过后会把 `windows-smartscreen-report-<tag>.md` 和 `windows-release-assets-<tag>.json` 上传到同一个 GitHub Release，作为公开可核验的审核证据。
...
- 如果 Windows 包开始被拦截，下载自动生成的 `windows-wdsi-package-<tag>` artifact，再提交签名后的 EXE/MSI 或 portable ZIP 到 https://www.microsoft.com/en-us/wdsi/filesubmission。

## Microsoft Defender 发布前扫描

`Windows Release` 和 `Windows Store Package` 会在上传产物前运行 `scripts/scan-windows-assets.ps1`。该脚本使用 GitHub Windows runner 上的 Microsoft Defender `MpCmdRun.exe` 对 Windows Release 的 EXE/MSI/portable ZIP 和 Store 用 EXE/MSI 做自定义扫描，扫描失败或检测不可用都会让 workflow 失败。

`Windows Release Audit`、`Windows Installer Smoke Test`、`Windows WinGet Manifest` 和 `Windows WDSI Package` 还会重新下载 Release 里的 EXE/MSI/portable ZIP 并再次运行 Defender 扫描。这样可以覆盖构建时文件没问题，但 Release 资产被替换或重新上传后没有再扫的风险。
...
运行仓库脚本验证指定 Release 的 Windows 安装包和 portable ZIP：
...
脚本会下载 Release 中的 Windows EXE/MSI/portable ZIP，生成：
...
`cap-v*` Release published 后会自动触发 `Windows Release Audit`、`Windows Installer Smoke Test`、`Windows WinGet Manifest` 和 `Windows WDSI Package`。由 `Windows Release` workflow 创建正式签名 Release 时，会先创建 draft，再用 `workflow_dispatch` 主动触发并等待这些后续 workflow，避免 GitHub Actions 自己创建 Release 后没有继续触发证据链；触发时会传入父 run id，等待时只匹配同一个父 run id 的后续检查，避免并发手动重跑导致误判。任一后续 workflow 失败都会让 Release 保持 draft，全部通过后，父 workflow 还会重新读取 GitHub Release 资产，确认 EXE、MSI、portable ZIP、`SHA256SUMS.txt`、`windows-smartscreen-report-<tag>.md`、`windows-release-assets-<tag>.json`、`windows-installer-smoke-test-report-<tag>.md`、`windows-installer-smoke-test-results-<tag>.json`、`windows-winget-manifest-<tag>.zip`、`windows-winget-submission-<tag>.md`、`windows-wdsi-submission-checklist-<tag>.md` 和 `windows-wdsi-submission-text-<tag>.zip` 都已经存在，并用 quarantine 脚本再次校验 `windows-release-assets-<tag>.json` 中每个 Windows 安装包和 portable ZIP 状态都有效，然后才会自动公开。也可以在 GitHub Actions 里手动运行它们并输入 Release tag。Release 审计和 WinGet 生成器通过 GitHub Release Asset API 下载资产，所以正式 Release 还处于 draft 门禁阶段时也能读取安装包。Release 审计会用同一个脚本检查 Release，要求 Windows 包签名有效、带可信时间戳、通过 `signtool verify /pa /tw`、匹配 Release 中的 `SHA256SUMS.txt`、通过 GitHub artifact attestation 验证，并且通过 Microsoft Defender 扫描。安装器 smoke test 会在干净 Windows runner 上静默安装和卸载 EXE/MSI；portable ZIP 不进入安装器 smoke test，而是在 Release Audit 中展开验证内部 EXE/DLL。只有这些检查通过后，才建议把 GitHub Release 链接发给普通用户、提交 WinGet 或用于 WDSI 申诉。

当前 `cap-v0.4.3-cn` 已被隔离为 prerelease；其中的 Windows EXE/MSI 仍未签名，portable ZIP 已补上但同样缺少已签名内部 EXE/DLL 和完整 release 证据门禁。启用任一签名后端并重新发布后，应重新运行该脚本并确认状态为 `Valid`。

`Windows Release Quarantine` 会在 Release 发布或编辑时自动检查是否仍挂着未验证 Windows EXE/MSI/portable ZIP。它不只检查证据资产名称是否存在，还会下载并解析 `windows-release-assets-<tag>.json`，确认每个 Windows 安装包和 portable ZIP 的签名、时间戳、SignTool、checksum、artifact attestation 和 Defender 扫描状态都有效。自动触发时，如果 Release 带有 unsafe Windows 包，会把该 Release 标记为 prerelease，避免它继续作为普通最新版本被用户下载。

`Windows Release Quarantine` 也会在 `main` 更新后和每天定时扫描所有普通公开 `cap-v*` Release，通过 `scripts/protect-public-windows-releases.ps1` 把仍带 unsafe Windows 包的旧 Release 自动标记为 prerelease。如果要手动隔离旧的未验证 Windows Release 资产，也可以手动运行该 workflow。默认 `mode=report` 只生成报告；如果确认要降低旧 Release 被普通用户误下载的概率，可以把 `mode` 设为 `mark_prerelease` 并输入 `mark-prerelease:<tag>`，或把 `mode` 设为 `delete_windows_assets` 并输入 `delete-windows-assets:<tag>` 删除该 Release 上的 Windows EXE/MSI/portable ZIP 资产。不要对已经通过签名、checksum、Release audit、installer smoke test、WinGet 和 WDSI 证据门禁的 Release 使用删除模式。

Windows Release 的内部传递 artifact 和 SignPath unsigned 中间 artifact 只保留 1 天；Store/MSIX 提交 artifact 只保留 14 天，避免 Actions artifacts 变成长期公开下载渠道。GitHub Actions artifacts 只用于 workflow 内传递或 Partner Center 提交，不作为普通用户下载入口。
...
7. 为每个 EXE/MSI/portable ZIP 生成一份 WDSI 提交说明文本
8. 打包 Windows 包、报告、hash、证据 JSON 和 checklist

下载 artifact `windows-wdsi-package-<tag>` 后，在 https://www.microsoft.com/en-us/wdsi/filesubmission 选择 `Software developer`，上传 `installers` 目录中的对应 Windows 包，并粘贴 `submission-text` 目录中同名文本。如果使用 Release 资产中的 WDSI 提交文本压缩包，则上传 Release 中同名 EXE/MSI/portable ZIP，并粘贴压缩包里对应文件名的文本。
...
10. 配置 Windows 签名后，在 GitHub Actions 手动运行 `Windows Store Package`。
...
18. EXE/MSI 产品已经创建并完成过首次提交后，可以重新运行 `Windows Store Package`，把 `publish_to_store` 设为 `true`，填写 `store_product_id` 和 `package_url_base`，让 workflow 使用 `microsoft-store-product-update.json` 自动更新并提交 Store 草稿包。微软 Store submission API 不能创建新的 Partner Center 应用，也要求先有一次人工提交。
19. 提交审核。
