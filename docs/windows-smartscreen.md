...
`Windows Release` workflow 默认要求 Windows Authenticode 签名。通过 `cap-v*` tag 触发的正式 Release 不能关闭签名要求；手动运行 workflow 时，只有把 `require_signing` 输入显式设置为 `false`，才会允许生成未签名测试包。把 `WINDOWS_SIGNING_PROVIDER` 设置为 `azure-artifact-signing` 或 `pfx` 后，Windows 构建会：
