...
2. 申请 SignPath Foundation 免费开源代码签名。
...
当前 `cap-v0.4.3-cn` 已被隔离为 prerelease；其中的 Windows EXE/MSI 验证结果是 `NotSigned`，且还没有新的 portable ZIP。启用任一签名后端并重新发布后，应重新运行该脚本并确认状态为 `Valid`。
