...
2. Azure Artifact Signing（formerly Trusted Signing）
...
3. SignPath
   ...
   SignPath 是另一个适合 CI 的签名服务，开源项目可以申请 SignPath Foundation。仓库已有 `.github/signpath/artifact-configuration.xml`，会对 Windows 主程序、DLL 和 GitHub Release 中的 Windows EXE/MSI 执行 Authenticode 签名；portable ZIP 会在主程序/DLL 已签名后生成。
