...
`Windows Release` workflow 默认仍允许发布未签名包，方便没有证书时继续生成测试安装包。把 `WINDOWS_REQUIRE_SIGNING` 设置为 `true` 后，Release workflow 会拒绝生成未签名 Windows 包。把 `WINDOWS_SIGNING_PROVIDER` 设置为 `azure-artifact-signing` 或 `pfx` 后，Windows 构建会：
...
| `WINDOWS_REQUIRE_SIGNING` | Optional, set `true` to block unsigned Release builds |
...
配置好任一签名后端后，先手动运行 GitHub Actions 中的 `Windows Signing Check` workflow。它不会构建安装包，只检查 `WINDOWS_SIGNING_PROVIDER` 与对应 Variables/Secrets 是否齐全。检查通过后再创建新的 `cap-v*` tag 或手动运行 `Windows Release`。
