...
1. Microsoft Store
   ...
   运行 GitHub Actions 中的 `Windows Store Package` workflow 可以生成 Store 用离线安装包。默认情况下该 workflow 要求 Windows 签名已配置；如需测试构建，可以手动把 `require_signing` 设为 `false`。workflow 还会生成 `store-submission-package`，把 Partner Center 需要的包 URL、架构、语言、静默安装参数、SHA256、签名状态和发布者信息整理成 checklist、JSON 和 CSV。
...
4. 如果已经有正式签名 Release 或 CDN 下载目录，运行 workflow 时把 `package_url_base` 填成版本化 HTTPS 前缀；也可以先留空，等安装包上传到固定 URL 后再补。
5. 下载 workflow artifact `windows-store-package-<version>`。
6. 在 Partner Center 链接离线安装包。Microsoft Store 的 MSI/EXE 路线要求版本化 HTTPS 包 URL，不应在审核开始后替换同一 URL 下的二进制文件。
7. 参考 `store-submission-package/microsoft-store-submission-checklist.md`、`microsoft-store-submission.json` 或 `microsoft-store-packages.csv` 填写架构、语言、hash、签名和发布者信息。
8. 如果使用 NSIS EXE，静默安装参数填写 `/S`。
9. 如果使用 MSI，Partner Center 可以使用默认 `/qn` 静默安装；本仓库 smoke test 使用 `/quiet /norestart` 复核本地安装器行为。
10. 确认同版本 Release 自动触发的 `Windows Installer Smoke Test` 已通过，必要时手动重跑。
11. 提交审核。
