...
- 公开签名 Release 创建后，`Windows Release` 会主动触发 Release 审计、安装器 smoke test、WinGet manifest 和 WDSI 证据包 workflow。
...
- `Windows Release Audit` 会在 `cap-v*` Release published 后自动审计；由 `Windows Release` workflow 创建公开签名 Release 时，也会被主动触发。
