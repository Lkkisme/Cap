...
- Windows Release、Windows Store EXE/MSI 和 Windows MSIX Store 构建都会设置 `VITE_DISABLE_UPDATER=true`，桌面端不会尝试走旧的内置 updater；普通生产 Tauri 配置也不再指向上游 CrabNebula updater。手动更新入口会打开本仓库 Releases，web dashboard 的 Download App 菜单会打开当前站点 `/download`，继续走 Windows Store 优先和证据门禁。
