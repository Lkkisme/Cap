...
- Web 下载入口的 Windows 路由已改为读取本仓库 `Lkkisme/Cap` 的 GitHub Release；`/download/windows` 和 `/download/windows-msi` 只会直跳带 `SHA256SUMS.txt` 的 Release 资产，否则回到 Releases 页面，避免把旧的未签名包作为推荐下载。
