...
- GitHub 仓库：公开仓库
- 主要发布目标：Windows x64
- 推荐下载入口：网站 `/download/windows`，配置 Microsoft Store URL 后优先跳 Store；未配置 Store 时只会跳证据齐全的已签名 Windows Release
...
- 当前还没有可作为普通用户推荐下载的已签名 Windows Release；需要先完成 Microsoft Store 上架或配置真实 Windows 代码签名后重新发布
...
2. 使用 Azure Artifact Signing（formerly Trusted Signing）、SignPath 或代码签名证书签名 Windows EXE/MSI，并确保 portable ZIP 内的 EXE/DLL 也来自已签名产物且带可信时间戳。
...
- Web 下载入口的 Windows 路由已改为优先使用 Microsoft Store；部署时配置 `NEXT_PUBLIC_WINDOWS_STORE_URL`、`WINDOWS_STORE_URL` 或 `CAP_WINDOWS_STORE_URL` 为官方 Microsoft Store HTTPS 详情页或应用页链接后，`/download/windows` 会跳到 Store。未配置 Store 链接时，`/download/windows`、`/download/windows-exe`、`/download/windows-msi` 和 `/download/windows-zip` 只会直跳同时带 Windows EXE、MSI、portable ZIP、`SHA256SUMS.txt`、当前 tag 对应的 `windows-smartscreen-report-<tag>.md`、`windows-release-assets-<tag>.json`、`windows-installer-smoke-test-report-<tag>.md`、`windows-installer-smoke-test-results-<tag>.json`、`windows-winget-manifest-<tag>.zip`、`windows-winget-submission-<tag>.md`、`windows-wdsi-submission-checklist-<tag>.md` 和 `windows-wdsi-submission-text-<tag>.zip`，并且 `windows-release-assets-<tag>.json` 中每个 Windows 安装包和 portable ZIP 的签名、时间戳、SignTool、checksum、attestation 和 Defender 状态都有效的 Release 资产；配置 `WINDOWS_RELEASE_ASSET_BASE_URL`、`CAP_WINDOWS_RELEASE_ASSET_BASE_URL` 或 `NEXT_PUBLIC_WINDOWS_RELEASE_ASSET_BASE_URL` 后，这些已验证 Windows 资产会优先使用你自己的版本化 HTTPS CDN 或网站路径，而不是直接把用户送到 GitHub Release。没有可信 Windows 包时会进入 `/download/windows-status`，避免把旧的未签名包、未完成审计的包或未通过安装器 smoke test/WinGet/WDSI 材料生成的包作为推荐下载。All Versions 页面也只会给证据齐全且清单内容有效的 Windows Release 展示 EXE/MSI/ZIP 直链。
...
- Windows Release、Windows Store EXE/MSI 和 Windows MSIX Store 构建都会设置 `VITE_DISABLE_UPDATER=true`，桌面端不会尝试走旧的内置 updater；普通生产 Tauri 配置也不再指向上游 CrabNebula updater。手动更新入口和 web dashboard 的 Download App 菜单都会打开当前站点 `/download` 或 `/download/versions`，继续走 Windows Store 优先和证据门禁。
