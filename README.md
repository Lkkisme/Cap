...
- 旧 Release：[cap-v0.4.3-cn](https://github.com/Lkkisme/Cap/releases/tag/cap-v0.4.3-cn) 已被标记为 prerelease，因为其中的 Windows EXE/MSI 尚未签名，且还不是带 portable ZIP 的新发布
- 当前还没有可作为普通用户推荐下载的已签名 Windows Release；需要先完成 SignPath Foundation 或其它真实 Windows 代码签名后重新发布
- 当前 Windows SmartScreen 处理目标已收敛为申请 SignPath Foundation 免费开源代码签名；申请材料见 [docs/signpath-foundation-application.md](docs/signpath-foundation-application.md)，公开签名政策见 [CODE_SIGNING_POLICY.md](CODE_SIGNING_POLICY.md)，隐私政策见 [PRIVACY.md](PRIVACY.md)
...
本 fork 的目标是申请 SignPath Foundation 免费开源代码签名，用 SignPath Foundation 的公开证书签 Windows EXE、MSI 和 portable ZIP 内部的 EXE/DLL。这样公司电脑看到的发布者会是可信代码签名发布者，而不是完全未签名文件。
...
1. 确认 GitHub 账号已开启 MFA。
2. 确认本仓库保持公开 fork，且源码、workflow、许可证、README、隐私政策和 Code signing policy 都可公开访问。
3. 使用 [docs/signpath-foundation-application.md](docs/signpath-foundation-application.md) 中的文案提交 SignPath Foundation 申请。
4. 申请通过后，在 SignPath 中创建或确认 project、release signing policy、artifact configuration 和 GitHub trusted build system。
5. 在 GitHub 仓库 Variables 中设置 `WINDOWS_SIGNING_PROVIDER=signpath`、`WINDOWS_SIGNING_PUBLISHER_PATTERN=SignPath Foundation` 和下载入口相关变量。
6. 在 GitHub 仓库 Secrets 中设置 `SIGNPATH_API_TOKEN`、`SIGNPATH_ORGANIZATION_ID`、`SIGNPATH_PROJECT_SLUG`、`SIGNPATH_SIGNING_POLICY_SLUG` 和 `SIGNPATH_ARTIFACT_CONFIGURATION_SLUG`。
7. 手动运行 `Windows Signing Check`，确认临时 EXE 签名探针状态为 `Valid`。
8. 创建新的 `cap-v*` tag 或手动运行 `Windows Release`，生成新的已签名 Windows EXE、MSI 和 portable ZIP。
9. 等待 `Windows Release Audit`、`Windows Installer Smoke Test`、`Windows WinGet Manifest` 和 `Windows WDSI Package` 全部通过。
10. 下载 EXE/MSI/portable ZIP；EXE/MSI 可直接用 `Get-AuthenticodeSignature` 确认签名为 `Valid`，ZIP 解压后对里面的 `Cap-CN.exe` 和 DLL 做同样检查，并确认发布者是 SignPath Foundation 且存在可信时间戳。
